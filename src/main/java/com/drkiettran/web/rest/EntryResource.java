package com.drkiettran.web.rest;

import com.codahale.metrics.annotation.Timed;
import com.drkiettran.domain.Entry;

import com.drkiettran.repository.EntryRepository;
import com.drkiettran.security.SecurityUtils;
import com.drkiettran.web.rest.errors.BadRequestAlertException;
import com.drkiettran.web.rest.util.HeaderUtil;
import com.drkiettran.web.rest.util.PaginationUtil;
...
public class EntryResource {
	 ...
	 *         entry, or with status 400 (Bad Request) if the entry has already an
	 *         ID
	 ...
	 *         entry, or with status 400 (Bad Request) if the entry is not valid, or
	 *         with status 500 (Internal Server Error) if the entry couldn't be
	 *         updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Entry> updateEntry(@Valid @RequestBody Entry entry) throws URISyntaxException {
		...
		Entry result = entryRepository.save(entry);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, entry.getId().toString()))
				.body(result);
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and the list of entries in
	 *         body
	 */
	@GetMapping("/entries")
	@Timed
	public ResponseEntity<List<Entry>> getAllEntries(Pageable pageable) {
		...
		Page<Entry> page = entryRepository.findByBlogUserLoginOrderByDateDesc(SecurityUtils.getCurrentUserLogin(),
				pageable);
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the entry, or
